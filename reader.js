(() => {
  ...
  function updateJobUi() {
    ...
    if (reviewJob) { const label = MaxQuillReviewApi.STATUS_LABELS[reviewJob.status]; queue.textContent = `Status: ${label}`; document.querySelector("#review-chapter-status").textContent = `Review complete · Submitted · ${label}`; if (reviewJob.status === "REVISION_READY") queue.className = "is-ready"; if (reviewJob.status === "FAILED") queue.className = "is-failed"; }
