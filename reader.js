(() => {
  ...
  function updateJobUi() {
    ...
    if (reviewJob) { queue.textContent = `Queue status: ${MaxQuillReviewApi.STATUS_LABELS[reviewJob.status]}`; if (reviewJob.status === "REVISION_READY") queue.className = "is-ready"; if (reviewJob.status === "FAILED") queue.className = "is-failed"; }
    submit.hidden = Boolean(reviewJob); submit.disabled = !review.completed || submitting; submit.textContent = submitting ? "Submitting..." : "Submit for Revision";
  }
  ...
  async function submitReview() {
    ...
    submitting = true; updateJobUi();
    try { const job = await MaxQuillReviewApi.submitOwnerReview(buildOwnerReviewPackage(), sourcePackage); persistJob(job); showMessage("#review-message", `Submitted. ${MaxQuillReviewApi.STATUS_LABELS[job.status]}.`); await refreshJobStatus(); startPolling(); }
    catch (error) { showMessage("#review-message", submissionError(error)); }
    finally { submitting = false; updateJobUi(); }
  }
  ...
  async function init() {
    setupSettings(); try {
      if (!window.MaxQuillReviewContract || !window.MaxQuillSelectionLogic || !window.MaxQuillReviewApi) throw new Error("Review validation support is unavailable."); const bookResponse = await fetch(BOOK_URL); if (!bookResponse.ok) throw new Error(`Book data returned ${bookResponse.status}`); book = await bookResponse.json();
