export class D1JobStore {
  ...
  async byPackage(bookId, chapterId, chapterVersion, packageFingerprint) { return this.db.prepare("SELECT * FROM review_jobs WHERE book_id = ? AND chapter_id = ? AND chapter_version = ? AND package_fingerprint = ?").bind(bookId, chapterId, chapterVersion, packageFingerprint).first(); }
  async byId(id) { return this.db.prepare("SELECT * FROM review_jobs WHERE id = ?").bind(id).first(); }
  async create(job) {
    await this.db.prepare("INSERT INTO review_jobs (id, book_id, chapter_id, chapter_number, chapter_version, status, package_fingerprint, review_fingerprint, review_package_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 'QUEUED', ?, ?, ?, ?, ?)").bind(job.id, job.bookId, job.chapterId, job.chapterNumber, job.chapterVersion, job.packageFingerprint, job.reviewFingerprint, job.reviewJson, job.now, job.now).run();
